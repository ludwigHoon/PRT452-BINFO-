...
alleles <- function(loci_info) {
    ...
    for(i in 1:length(filenames)) {
        ...
        gene <- strsplit(as.character(getName(correct_length)), '_')
        gene_name <- sapply(gene, `[[`,1)
	    gene_number <- sapply(gene, `[[`,2)

        fragment <- getSequence(getFrag(correct_length, info[i,4], info[i,5]), as.string=TRUE)
        fragment_STR <- sapply(fragment, `[[`, 1)
        gc_content <- sapply(getFrag(correct_length, info[i,4], info[i,5]), GC) # returns between 0 and 1
        len <- sapply(getFrag(correct_length, info[i,4], info[i,5]), length)
        curve <- gc_content*len
